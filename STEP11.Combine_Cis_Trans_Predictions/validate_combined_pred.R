...
testing_prediction_folder = "CMC_EUR"
dataset_testing         = "CMC"
dataset_training        = "LIBD"

load_testing_data = function(data_path = './rnaseq/cmc_rnaseq.RData', 
                             tissues    = c("dlpfc","acc")) {
...
analyze_predictions = function(model_type, dataset_training, testing_prediction_folder, name_target, 
  ...
  config = list(
    base_path = "./predictions/%s_training/%s/",
    file_pattern = "%s_MIEC_predictions.txt",
    output_pattern = "Combined_perf_%s_MIEC_%s.RData"
  )
  ...
  process_region = function(region, model = NULL, dataset_training, testing_prediction_folder, name_target) {
    ...
    pred_expr_file_name = sprintf(config$file_pattern, region)
    
    ## output file name
    output_file = file.path(pred_expr_file_path, sprintf(config$output_pattern, region, name_target))
...
main = function() {
  ...
  # Analyze cis/trans combined predictions
  combined_results = analyze_predictions(
    model_type    = "Combined", dataset_training = dataset_training, testing_prediction_folder = testing_prediction_folder, name_target = dataset_testing,
