#####################################################################
# Brain Tissue Gene Expression Prediction Training Pipeline (Basic Elastic Net)
#####################################################################
...
## HELPER FUNCTIONS
###############
source("./STEP2.Training_cis/training_helper_functions.R")
...
CONFIG   = list(
  ...
  output_dir          = file.path(base_dir, "/STEP2.Training_cis/output_CIS/"),
  ...
  weights_cols = c(
    ...
    'distTSS', 'rsid', 'refAllele', 'effectAllele', 'weight', 'alpha'
  ),
  ...
  file_patterns = list(
    ...
    eqtl          = "chr%d_significant_ciseQTL.Rds", ## to rename
...
run_training_pipeline = function(CONFIG) {
  ...
  for(brain_region in CONFIG$brain_regions) {
    ...
    for(chr in CONFIG$chromosomes) {
      process_chromosome(brain_region, chr, data_region, CONFIG)
    }
