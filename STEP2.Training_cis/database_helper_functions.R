...
makeModelDB1 = function(brain.region, file_input_summ, file_input_weights, 
                        drv_file_path, dataset_training, model) {
  
  # Read summary file for network
  summary = read.table(file.path(file_input_summ, brain.region, paste0(brain.region,"_all_chromosomes_summary.txt")), header = TRUE, stringsAsFactors = FALSE)
  weights = read.table(file.path(file_input_weights, brain.region, paste0(brain.region, "_all_chromosomes_weights.txt")), header = TRUE, stringsAsFactors = FALSE)
  
  # Create directory for database files if it doesn't exist
  if (!dir.exists(file.path(drv_file_path,brain_region))) {
    dir.create(file.path(drv_file_path,brain_region), recursive = TRUE)
  }
  ## Assign name to DB file
  db_file = file.path(drv_file_path, brain.region,
                      paste0(model, "_",brain.region, '_', dataset_training, '.db'))
 ...
 if(file.exists(db_file)) {
  ...
  res = makeModelDB2(
    ...
    summary            = summary,
    weights            = weights,
    saved_db_file_path = file.path(drv_file_path,brain_region),
...
makeModelDB2 = function(brain.region, summary, weights, 
                        saved_db_file_path, output_drv_name) {
  
  #########################################
  # Filter model with significant results
  #########################################
  model_summaries = summary  %>% 
    dplyr::rename(genename = "gene_name",  n.snps.in.model = "n_snps_in_model", gene = "gene_id")
  
  model_summaries$pred.perf.qval = model_summaries$pred.perf.pval = model_summaries$pred.perf.R2 = NA
  ...
  weights = weights %>% dplyr::filter(gene %in% model_summaries$gene) %>%
    dplyr::rename(ref_allele = refAllele, eff_allele = effectAllele) 
