#' CIS Genotype Matching Script
#' 
#' This script performs genotype matching for EpiXcan analysis by:
#' 1. Loading and processing genotype data from bim files
#' 2. Matching SNPs with EpiXcan reference database
...
CONFIG    = list(
  ...
  output  = list(
    directory  = "./genotype/EpiXcan.matched.geno/"
  ),
  tools   = list(
    plink2_path = "../../tools/plink2"
  ),
  database  = list(
    base_path = "./PredictDB_LD/EpiXcan/folder_containing_your_training_DBs",
    suffix    = "EpiXcan.LD"
  ),
  temp_files = list(
    directory        = "./temp/",
    to_extract       = "EpiXcan.to.extract.txt",
    to_update_allele = "EpiXcan.to.update.allele.txt",
    to_swap_allele   = "EpiXcan.to.swap.allele.txt"
  )
)
...
#' Main processing function for EpiXcan matching
...
process_epixcan_match = function(brain_region, input, input_dir, target, output_dir, config) {
  ...
  # STEP 2: Load EpiXcan reference database
  print("matching with EpiXcan ref")
  sqlite.driver = dbDriver("SQLite")
  db_path = file.path(config$database$base_path, input, brain_region, paste0(config$database$suffix, ".",target,".db"))
  ...
  if(!length(Reduce(union,list(com.snps,recode.snps,com.recode)))){
    cmd.1 = paste0(config$tools$plink2_path, 
                   ...
                   " --export vcf --out ", output_dir, input, "_", brain_region, "_EpiXcan.matched")
    system(cmd.1)
  ...
  if(length(union(com.snps,com.recode)) & !length(union(recode.snps,com.recode))){
    ...
    cmd.2 = paste0(config$tools$plink2_path, " --bfile ", output_dir, input, ".extract",
                   " --update-alleles ", update_path, " --recode vcf --out ", output_dir, input, "_", brain_region,  "_EpiXcan.matched")
    system(cmd.1)
  ...
  if(!length(union(com.snps,com.recode)) & length(union(recode.snps,com.recode))){
    ...
    cmd.1 = paste0(config$tools$plink2_path, " --bfile ", input_dir, input,
                   " --extract ", extract_path, " --make-bed --out ", output_dir, input, ".extract")
    cmd.2 = paste0(config$tools$plink2_path, " --bfile ", output_dir, input, ".extract",
                   " --alt1-allele ", swap_path, " 5 1 --recode vcf --out ", output_dir, input,  "_", brain_region, "_EpiXcan.matched")
    system(cmd.1)
    print("extraction completed")
    system(cmd.2)
