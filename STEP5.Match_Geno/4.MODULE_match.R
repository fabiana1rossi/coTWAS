#' MODULE Genotype Matching Script
#' 
#' This script performs genotype matching for MODULE analysis by:
#' 1. Loading and processing genotype data from bim files
#' 2. Matching SNPs with MODULE reference databases
...
CONFIG    = list(
  inputs = list(
    file       = "your_testing_genotype_prefix",
    directory  = "./genotype/",  # Make sure this directory exists with your input files
    target     = "your_testing_geno",
    regions    = c("dlpfc")
  ),
  output  = list(
    directory        = "./genotype/MODULE.matched.geno/"  # This will be created automatically
  ),
  tools   = list(
    plink2_path      = "../../tools/plink2"  # Make sure plink2 is in your system PATH
  ),
  database  = list(
    base_path        = "./PredictDB_LD/MODULE/folder_containing_your_training_DBs/",  # Make sure this directory exists with your reference files
    suffix           = ".extras_weights.RData"
  ),
  temp_files = list(
    directory        = "./temp/",  # This will be created automatically
    to_extract       = "MODULE.to.extract.txt",
    to_update_allele = "MODULE.to.update.allele.txt",
    to_swap_allele   = "MODULE.to.swap.allele.txt"
  )
)
...
#' Main processing function for MODULE matching
#' @param brain_region Brain region identifier
...
process_module_match = function(brain_region, input, input_dir, target, output_dir, config) {
  ...
  # STEP 2: Load MODULE reference database
  print("matching with MODULE ref")
  db_path = file.path(config$database$base_path,paste0("MODULE.",brain_region, config$database$suffix))
  load(db_path)
  rm(extras);gc()
  df      = weights
  ...
  df     = df[,c("ID","varID","REF","EFF")]
  df     = distinct(df)
  df$CHR = as.numeric(gsub("chr","",limma::strsplit2(df$varID,":")[,1])) #### TO CHANGE
  ...
  if(!length(Reduce(union,list(com.snps,recode.snps,com.recode)))){
    cmd.1 = paste0(config$tools$plink2_path, 
                   ...
                   " --export vcf --out ", output_dir, input, "_MODULE.matched")
    system(cmd.1)
  ...
  if(length(union(com.snps,com.recode)) & !length(union(recode.snps,com.recode))){
    ...
    cmd.2 = paste0(config$tools$plink2_path, " --bfile ", output_dir, input, ".extract",
                   " --update-alleles ", update_path, " --recode vcf --out ", output_dir, input,  "_", brain_region, "_MODULE.matched")
    system(cmd.1)
  ...
  if(!length(union(com.snps,com.recode)) & length(union(recode.snps,com.recode))){
    ...
    cmd.1 = paste0(config$tools$plink2_path, " --bfile ", input_dir, input,
                   " --extract ", extract_path, " --make-bed --out ", output_dir, input, ".extract")
    cmd.2 = paste0(config$tools$plink2_path, " --bfile ", output_dir, input, ".extract",
                   " --alt1-allele ", swap_path, " 5 1 --recode vcf --out ", output_dir, input, "_", brain_region, "_MODULE.matched")
    system(cmd.1)
    print("extraction completed")
    system(cmd.2)
