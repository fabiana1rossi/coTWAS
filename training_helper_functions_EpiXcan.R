...
get_file_path = function(file_type, brain_region=NULL, chr=NULL, CONFIG) {
  ...
  } else if(file_type %in% c("prior")) {
    return(sprintf(file.path(CONFIG$prior_dir, pattern), brain_region))
...
nested_cv_elastic_net_perf = function(x, y, pfac, n_samples, train_test_fold_ids, alpha, n_k_folds=4) {
...
#' Train elastic net model for a gene
...
train_elastic_net_nested= function(gene_id, expression, genotypes, covariates=NULL, brain_region, CONFIG) {
  # Prepare data
  y = expression[, gene_id,drop=FALSE]
  if(!is.null(covariates)) {
    y = adjust_for_covariates(y, covariates)
  }
  
  # Cross-validation
  cv_results = nested_cv_elastic_net_perf(
    x                  = genotypes,
    y                  = y,
    pfac               = rep(1, ncol(genotypes)),  # Default penalty factors
...
process_chromosome = function(brain_region, chr, data_region, CONFIG, annot_results, annot_bed) {
  ...
  ## Select cisEQTLs based on pvalue ( Does EpiXcan filter???)
  significant_ciseQTL = significant_ciseQTL %>% mutate(FDR = p.adjust(meta_p, method="fdr")) %>% filter(FDR < 0.05)
  ...
  ## Load pre-computed SNP annotation with Priors
  output_file = file.path(CONFIG$prior_dir, sprintf("%s_chr%s_annotations.Rds", brain_region, chr))
  
  if(!file.exists(output_file)){
    stop(sprintf("SNP annotation file not found: %s. Please run 1.create_SNP_annotation_lookup.R first.", output_file))
  }
  
  # Load SNP priors
  snp_prior = readRDS(output_file)
  snp_prior$Prior[snp_prior$Prior<0] = 0
...
train_gene_model = function(gene, gene_annot, expression, genotypes, covariate, snp_annot, ciseqtl, snp_prior, brain_region, CONFIG, annot_results) {
  # Input validation
  ...
  ################
  ## From EpiXcan : https://bitbucket.org/roussoslab/epixcan/src/master/scripts/EpiXcan_CV_elasticNet_penalty.R
  ## Get cis-SNPs
  ...
  # Get SNP priors for cis-SNPs from pre-computed lookup
  cissnps_prior = snp_prior[snp_prior$RSID_dbSNP137 %in% cissnps$SNP, ]
  
  # Remove duplicates if any
  if(any(duplicated(cissnps_prior$RSID_dbSNP137))) {
    cissnps_prior = cissnps_prior[-which(duplicated(cissnps_prior$RSID_dbSNP137)), ]
  }
  
  # Check if we have any SNPs with priors
  if(nrow(cissnps_prior) == 0) {
    warning(sprintf("No SNPs with priors found for gene %s", gene))
    return(NULL)
  }
  
  # Set row names for lookup
  rownames(cissnps_prior) = cissnps_prior$RSID_dbSNP137
  
  # Pull cis-SNP genotypes
  cissnp1index = intersect(colnames(genotypes), cissnps$SNP)
  # get SNPs who have prior information
  index1 = intersect(cissnp1index, cissnps_prior$RSID_dbSNP137)
  cis_geno = genotypes[, cissnp1index, drop = FALSE]
  prior_penal = cissnps_prior[cissnp1index, ]
  
  # will consider all of the Cis-SNPs
  rownames(prior_penal) = cissnp1index
  # by default, priors are zeros
  prior_penal[,3] = rep(0, length(prior_penal[,3]))
  prior_penal[index1,3] = cissnps_prior[index1,3]
...
#' @param expression Expression data
#' @param genotypes Genotype data
#' @param pfac Penalty factors
#' @param original_priors Original prior probabilities
#' @param snp_annot SNP annotation
#' @param covariates Covariate data (not used - expression already adjusted)
#' @param brain_region Brain region name
#' @param CONFIG Configuration parameters
#' @return List with model results
train_elastic_net = function(gene_id, gene_annot, expression, genotypes, pfac, original_priors, snp_annot, covariates=NULL, brain_region, CONFIG, annot_results) {
  # Input validation
  ...
  # train_test_fold_ids = tryCatch({
  #   get(load(fold_file))
  # }, error = function(e) {
  #   stop(sprintf("Error loading CV fold file: %s", e$message))
  # })
  ...
  ## From EpiXcan https://bitbucket.org/roussoslab/epixcan/src/master/scripts/EpiXcan_CV_elasticNet_penalty.R
  ########
  #the optimal mapping is approximated by simulations, and is validated to be effective
  # default 0 priors will be transferred to penalty factor 1, which is embedded to cv.glmnet as default values
  # x2 is the maximal value of CMC priors
  # y2: best lower value of penalty factors
  #: (x1,y1): optimal coordinate of intermediate control point
  # simulation values   
  CMCx2=4.69250216
  # real max prior
  x2=max(annot_results$Weight)
  y2=0.7
  x1=1.9*x2/CMCx2
  y1=0.7
  ...
  original_priors[,6]=pfac
  # Although for PrediXcan, no priors are used, the information of each SNP including the prior is contained in the PredictDB
  colnames(original_priors)<-c('Chr', 'Pos','Prior', 'RSID_dbSNP137','Annotation','penalty_factor')
    
  # to get performance evaluations adapted from PrediXcan
  perf_measures <- nested_cv_elastic_net_perf(as.matrix(genotypes), (adj_expression), pfac, nrow(genotypes), train_test_fold_ids, 0.5, n_k_folds)
  ...
  fit <- tryCatch(cv.glmnet(as.matrix(genotypes),as.vector(adj_expression),penalty.factor=pfac, nfolds = n_k_folds, alpha = 0.5, type.measure='mse', foldid = cv_fold_ids, keep = TRUE),
  ...
  if (length(fit) > 0) {
    ...
    if (fit$nzero[best_lam_ind] > 0) {
      ...
      # Output best shrunken betas for EpiXcan
      ...
      bestpriorinfo<-original_priors[bestweightlist,]
      # calculate distTSS
